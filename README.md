...
## Отчёт о покрытии тестами 'Coverage'
### Для просмотра отчета покрытия тестами введите:
Для Windows :
```bash
...
```
Если HTML-отчёт отсутствует, то вы можете сгенерировать его самостоятельно, затем просмотреть, как показано выше.
```bash
coverage html
```
